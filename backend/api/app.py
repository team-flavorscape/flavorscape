...
class App:
    ...
    def post_onboard(self, req: OnboardRequest) -> ResultResponse:
        ...
        # Remove recipes with allergens from dataset
        names = self.recommender.recipe_dataset.data_pd.index
        print(f'pre-filter: {len(self.recommender.recipe_dataset.data_pd)}')
        ...
        names = [self.recipes.get_recipe(i).name for i in ids]
        self.recommender.recipe_dataset.data_pd = self.recommender.recipe_dataset.data_pd.drop(names)
        print(f'post-filter: {len(self.recommender.recipe_dataset.data_pd)}')
    ...
    def penalty_function(self, ids: list[int]) -> list[int]:
        hist = self.state.recipe_history

        penalties = np.zeros((len(ids)))
        ni = [self.recipes.get_recipe(i).name for i in ids]
        for h in hist:
            nh = self.recipes.get_recipe(h).name

            ris = self.recommender.recipe_dataset.get_reduced_recipe(ni)
            rh = self.recommender.recipe_dataset.get_reduced_recipe(nh)

            diff = ris - rh
            dist = np.sqrt(np.sum(diff * diff, axis=1))

            dist = np.clip(dist, 1, None)
            penalties -= 1 / (dist * self.state.RECIPE_HISTORY_COUNT)

        #print(penalties.sort_values())
        return penalties
    ...
    def get_recommended_recipes(self, count: int) -> list[Recipe]:
        ...
        penalties = self.penalty_function([self.recipes.get_recipe_by_name(n).id for n in names])
        preds += penalties

        indices = np.argsort(preds)[-count:].tolist()
        print(f'score: {preds[indices[0]]} pen: {penalties[indices[0]]} {"hist" if self.recipes.get_recipe_by_name(names[indices[0]]).id in self.state.recipe_history else ""}')

        self.state.add_recipes_to_history([self.recipes.get_recipe_by_name(names[i]).id for i in indices])

        return [self.recipes.get_recipe_by_name(names[i]) for i in indices]
